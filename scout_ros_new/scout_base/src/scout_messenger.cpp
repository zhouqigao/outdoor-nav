...
void ScoutROSMessenger::SetupSubscription()
{
    ...
    imu_status_publisher_ = nh_->advertise<sensor_msgs::Imu>("/imu_status", 5);
    angle_status_publisher_ = nh_->advertise<geometry_msgs::Point>("/angle_status", 5);
    // cmd subscriber
...
void ScoutROSMessenger::PublishStateToROS()
{
    ...
    static uint64_t count_imu = 0;
    sensor_msgs::Imu imu_msg;
    imu_msg.header.frame_id = "imu_link";
    imu_msg.header.stamp = ros::Time::now();
    imu_msg.header.seq = count_imu++;

    imu_msg.angular_velocity.x = state.imu_state.angle_dot.x;
    imu_msg.angular_velocity.y = state.imu_state.angle_dot.y;
    imu_msg.angular_velocity.z = state.imu_state.angle_dot.z;

    imu_msg.linear_acceleration.x = state.imu_state.acc.x;
    imu_msg.linear_acceleration.y = state.imu_state.acc.y;
    imu_msg.linear_acceleration.z = state.imu_state.acc.z;

    imu_msg.orientation.x = state.imu_state.quater.x;
    imu_msg.orientation.y = state.imu_state.quater.y;
    imu_msg.orientation.z = state.imu_state.quater.z;
    imu_msg.orientation.w = state.imu_state.quater.w;
    //std::cout << "imu_state.quater.z: " << state.imu_state.quater.z <<std::endl;
    //std::cout << "imu_msg.orientation.z: " << imu_msg.orientation.z <<std::endl;

imu_msg.orientation_covariance[0] = 0.001225;
    imu_msg.orientation_covariance[4] = 0.001225;
    imu_msg.orientation_covariance[8] = 0.001225;
...
;
imu_msg.linear_acceleration_covariance[0] = 0.0096;
    imu_msg.linear_acceleration_covariance[4] = 0.0096;
    imu_msg.linear_acceleration_covariance[8] = 0.0096;

    //imu_msg.orientation_covariance[0] = state.imu_state.angle.x;
    //imu_msg.orientation_covariance[1] = state.imu_state.angle.y;
    //imu_msg.orientation_covariance[2] = state.imu_state.angle.z;
    geometry_msgs::Point angle_xyz;
    angle_xyz.x = state.imu_state.angle.x;
    angle_xyz.y = state.imu_state.angle.y;
    angle_xyz.z = state.imu_state.angle.z;
...
*/
    imu_status_publisher_.publish(imu_msg);
    angle_status_publisher_.publish(angle_xyz); 
...
odom_msg.pose.covariance[21]=0.03;
odom_msg.pose.covariance[28]=0.03;
